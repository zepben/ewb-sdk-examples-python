...
```python
network.connect_terminals(es_t, hv_line_t1) # Connect Energy Source to the HV Line
network.connect_terminals(hv_line_t2, hv_fuse_t1) # Connect HV Line to the HV Fuse
network.connect_terminals(hv_fuse_t2, tx_t1) # Connect HV Fuse to the Power Transformer
network.connect_terminals(tx_t2, lv_fuse_t1) # Connect Power Transformer to the LV Fuse
network.connect_terminals(lv_fuse_t2, breaker_t1) # Connect LV Fuse to the Breaker
network.connect_terminals(breaker_t2, lv_line_t1) # Connect Breaker to the LV Line
network.connect_terminals(lv_line_t2, ec_t) # Connect LV Line to the Energy Consumer
```
