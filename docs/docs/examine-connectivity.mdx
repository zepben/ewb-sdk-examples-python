...
## Create a Network (Simple Example)
...
## Examine Connectivity of the Network
...
The code snippet `fancy_print_connected_equipment(tx)` prints the connectivity results for the Power Transformer (tx). Since no phase information is provided, the `default=None` is used.
...
The code snippet `fancy_print_connected_equipment(tx, phases=PhaseCode.N)` prints the connectivity results for the Power Transformer (tx), considering only the `N` phase.
...
The code snippet `fancy_print_connected_equipment(breaker, phases=PhaseCode.BC)` prints the connectivity results for the Breaker (breaker), considering only the `B` and `C` phases.
...
The following code prints the connectivity results for the terminal `lv_fuse_t2` with phases `ACN` using the `connected_terminals` function. Note that `connected_terminals` is essentially `connected_equipment` where only one terminal is considered.
