...
print(line, "added?", network.add(line))
print(breaker, "added?", network.add(breaker))

# Objects with duplicate mRIDs are not added.
invalid = EnergySource(mrid="acls_123")
print(invalid, "added?", network.add(invalid))
...
print("Identified object with mrid b_456:", network.get("b_456"))
...
print("Switch with mrid b_456 is open?", network.get("b_456", Switch).is_open())
...
for conducting_equipment in network.objects(ConductingEquipment):
    print("Conducting equipment:", conducting_equipment)
...
print("Number of switches:", network.len_of(Switch))
print("Number of conducting equipment:", network.len_of(ConductingEquipment))
...
print("Network has unresolved references?", network.has_unresolved_references())
print("plsi_789 has unresolved references?", network.has_unresolved_references("plsi_789"))
for unresolved_reference in network.get_unresolved_references_from("acls_123"):
    print("Unresolved reference from acls_123:", unresolved_reference)
for unresolved_reference in network.get_unresolved_references_to("plsi_789"):
    print("Unresolved reference to plsi_789:", unresolved_reference)

print("Number of unresolved references to plsi_789:", network.num_unresolved_references("plsi_789"))
print("Total unresolved references:", network.num_unresolved_references())
...
for name_type in network.name_types:
    print("Network has name type", name_type)
