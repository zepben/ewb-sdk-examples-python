...
async def reconnect_all_three_circuits():
    channel = connect_with_token(host=c["host"], rpc_port=c["rpc_port"], access_token=c["access_token"], ca_filename=c["ca_path"])
    client = UpdateNetworkStateClient(channel)

    event1 = SwitchStateEvent("event1", datetime.now(), "20747776", SwitchAction.CLOSE)
    event2 = SwitchStateEvent("event2", datetime.now(), "20747792", SwitchAction.CLOSE)
    event3 = SwitchStateEvent("event3", datetime.now(), "20747784", SwitchAction.CLOSE)
    result = await client.set_current_states(int(time.time()), [event1, event2, event3])
    print(f"Update status: {result}")


async def disconnect_all_three_circuits():
    channel = connect_with_token(host=c["host"], rpc_port=c["rpc_port"], access_token=c["access_token"], ca_filename=c["ca_path"])
    client = UpdateNetworkStateClient(channel)

    event1 = SwitchStateEvent("event1", datetime.now(), "20747776", SwitchAction.OPEN)
    event2 = SwitchStateEvent("event2", datetime.now(), "20747792", SwitchAction.OPEN)
    event3 = SwitchStateEvent("event3", datetime.now(), "20747784", SwitchAction.OPEN)
    result = await client.set_current_states(int(time.time()), [event1, event2, event3])
    print(f"Update status: {result}")


async def reconnect_one_circuit():
    channel = connect_with_token(host=c["host"], rpc_port=c["rpc_port"], access_token=c["access_token"], ca_filename=c["ca_path"])
    client = UpdateNetworkStateClient(channel)

    event = SwitchStateEvent("event1", datetime.now(), "20747776", SwitchAction.CLOSE)
...
async def disconnect_one_circuit():
    ...
    event = SwitchStateEvent("event1", datetime.now(), "20747776", SwitchAction.OPEN)
    result = await client.set_current_states(int(time.time()), [event])
    print(f"Update status: {result}")
...
def run_single_circuit():
    ...
    asyncio.run(counts_disconnected_circuits(["20747926-lvf"]))
    print()
    print("Disconnecting Switch 20747926...")
    print()
    ...
    print("Reconnecting...")
    print()
    asyncio.run(reconnect_one_circuit())
    print("Equipment counts after reconnection:")
    asyncio.run(counts_disconnected_circuits(["20747926-lvf"]))    # 03 circuit reconnected, counts should match 1


def run_all_circuits():
    asyncio.run(counts_disconnected_circuits(["20747914-lvf", "20747918-lvf", "20747922-lvf", "20747926-lvf"]))
    asyncio.run(disconnect_all_three_circuits())
    asyncio.run(counts_disconnected_circuits(["20747914-lvf", "20747918-lvf", "20747922-lvf", "20747926-lvf"]))
    asyncio.run(reconnect_all_three_circuits())
    asyncio.run(counts_disconnected_circuits(["20747914-lvf", "20747918-lvf", "20747922-lvf", "20747926-lvf"]))
